...
TextMesh::TextMesh(const std::string &text, const Font &font) : font(font) {
	...
	*/
	for (unsigned int i = 0; i < text.length(); ++i) {
		...
		/*float y = (float)(c%font.xCount);
		float x = (float)((c / font.xCount) % font.yCount); */
		...
		/*textureCoords[(i * 4)] = Vector2(x, (y));
		textureCoords[(i * 4) + 1] = Vector2((x +1), (y));
		textureCoords[(i * 4) + 2] = Vector2(x, (y+1));
		textureCoords[(i * 4) + 3] = Vector2((x + 1), (y + 1));*/
	}
